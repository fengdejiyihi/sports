...
import type { DailyNutritionTarget, DietPlan, FoodAnalysis, ManualMealDraft, MealEntry, MealType } from './lib/nutrition'
import { loadReminder, saveReminder } from './lib/reminders'
import type { ReminderSettings } from './lib/reminders'
import { ageFromBirthDate, emptyProfile, isValidWeight, textList, validateProfile } from './lib/profile'
import type { ProfileDraft } from './lib/profile'
import { askNutrition, cloudEnabled, deleteMeal, deleteWorkoutPlan, loadCheckinHistory, loadMeals, loadMeasurements, loadNutritionTarget, loadProfile, loadToday, loadWorkoutPlans, requestMagicLink, saveBodyWeight, saveCheckin, saveManualMeal, saveMeal, saveNutritionTarget, saveProfile, saveWorkoutPlan, subscribeToUserData, supabase, updateMeal } from './lib/supabase'
...
import { emptyWorkoutPlan, validateWorkoutPlan, weekdayLabels, weekdayOptions } from './lib/workouts'
...
function App() {
  ...
  return <div className="min-h-screen bg-[#f4f7f4] text-[#17211a]">
    ...
    <main className="mx-auto max-w-5xl px-5 py-8">
      ...
      {page === 'today' && <Today saved={saved} stat={stat} targetWeightKg={profile.targetWeightKg} userId={session?.user.id} onCheckin={() => { setCheckinDate(today); setPage('checkin') }} onNutrition={() => setPage('nutrition')} />}
...
function Today({ saved, stat, targetWeightKg, userId, onCheckin, onNutrition }: { saved: boolean; stat: { weight: string; waist: string; sleep: string }; targetWeightKg: string; userId?: string; onCheckin: () => void; onNutrition: () => void }) {
  const [meals, setMeals] = useState<MealEntry[]>([])
  const [target, setTarget] = useState<DailyNutritionTarget | null>(null)
  useEffect(() => {
    if (!userId) return
    void Promise.all([loadMeals(userId, today), loadNutritionTarget(userId, today)]).then(([nextMeals, nextTarget]) => { setMeals(nextMeals); setTarget(nextTarget) })
  }, [userId])
  const totals = mealTotals(meals)
  return <section><p className="text-sm font-semibold text-[#438263]">今天 · 减脂计划</p><h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">{targetWeightKg ? `今天，为 ${targetWeightKg} kg 前进一步` : '今天，为自己前进一步'}</h1><p className="mt-3 text-[#647268]">完成训练后打卡，身体数据会自动更新。</p>{saved && <p className="mt-5 rounded-xl bg-[#dff1e4] px-4 py-3 text-sm font-semibold text-[#1e6743]">今天的打卡已保存。</p>}<div className="mt-7 grid gap-4 lg:grid-cols-[1.45fr_1fr]"><article className="rounded-3xl bg-[#256a49] p-6 text-white shadow-sm"><h2 className="text-xl font-bold">今晚训练：力量 A</h2><p className="mt-3 max-w-xl text-[#d9f2e1]">深蹲、俯卧撑、划船、臀桥、平板支撑 · 预计 40 分钟</p><button className="mt-6 rounded-xl bg-[#cef1d9] px-4 py-2.5 text-sm font-bold text-[#17452e]" onClick={onCheckin} type="button">开始打卡</button></article><article className="rounded-3xl border border-[#dfe9e0] bg-white p-6 shadow-sm"><h2 className="text-lg font-bold">身体状态</h2><div className="mt-2 divide-y divide-[#edf2ed]"><SummaryRow label="体重" detail="今日记录" value={stat.weight ? `${stat.weight} kg` : '未记录'} /><SummaryRow label="腰围" detail="今日记录" value={stat.waist ? `${stat.waist} cm` : '未记录'} /><SummaryRow label="睡眠" detail="今日记录" value={stat.sleep ? `${stat.sleep} h` : '未记录'} /></div></article></div><article className="mt-4 rounded-3xl border border-[#dfe9e0] bg-white p-6 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-bold">今日饮食</h2>{target ? <p className="mt-2 text-sm text-[#647268]">{target.trainingDay ? '训练日' : '休息日'}目标 · 已记录 {meals.length} 餐</p> : <p className="mt-2 text-sm text-[#647268]">还未保存今日营养目标</p>}<div className="mt-4 flex flex-wrap gap-x-8 gap-y-2"><strong className="text-2xl">{Math.round(totals.calories)}{target && <span className="text-base text-[#647268]"> / {target.targetCalories}</span>} <span className="text-sm text-[#647268]">kcal</span></strong><strong className="text-2xl">{totals.protein.toFixed(1)}{target && <span className="text-base text-[#647268]"> / {target.proteinGrams}</span>} <span className="text-sm text-[#647268]">g 蛋白质</span></strong></div></div><button className="rounded-xl bg-[#256a49] px-4 py-2.5 text-sm font-bold text-white" onClick={onNutrition} type="button">{target ? '记录一餐' : '生成目标'}</button></div></article></section>
}
...
function WorkoutPlans({ userId }: { userId?: string }) {
  ...
  const hasActions = draft.items.length > 0
  const addAction = () => setDraft({ ...draft, items: [...draft.items, { exerciseName: '', sets: '', repsMin: '', repsMax: '' }] })
  return <section className="max-w-3xl"><p className="text-sm font-semibold text-[#438263]">训练安排</p><h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">我的训练计划</h1><p className="mt-3 text-[#647268]">一个计划对应一个训练日；保存后可在每日打卡中选择。</p>{message && <p className="mt-5 rounded-xl bg-[#fff7df] px-4 py-3 text-sm text-[#765b18]" role="status">{message}</p>}<div className="mt-7 space-y-4">{plans.map((plan) => <article className="rounded-3xl border border-[#dfe9e0] bg-white p-6 shadow-sm" key={plan.id}><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-bold">{plan.name}</h2><p className="mt-2 text-sm text-[#647268]">{weekdayLabels[Number(plan.weekday)]} · {plan.durationMinutes} 分钟 · {plan.isActive ? '已启用' : '已停用'}</p><p className="mt-3 text-sm text-[#526055]">{plan.items.length ? plan.items.map((item) => `${item.exerciseName} ${item.sets}×${item.repsMin}${item.repsMin === item.repsMax ? '' : `–${item.repsMax}`}`).join(' · ') : '按时长训练 · 无固定动作'}</p></div><div className="flex gap-3"><button className="text-sm font-semibold text-[#256a49]" onClick={() => edit(plan)} type="button">编辑</button><button className="text-sm font-semibold text-[#a13d2e]" disabled={saving} onClick={() => { void remove(plan.id) }} type="button">删除</button></div></div></article>)}{!plans.length && <article className="rounded-3xl border border-dashed border-[#cbdccb] bg-white p-6 text-sm text-[#647268]">还没有计划。先创建一个常用训练安排吧。</article>}</div><form className="mt-4 rounded-3xl border border-[#dfe9e0] bg-white p-6 shadow-sm" onSubmit={submit}><h2 className="text-xl font-bold">{editingId ? '编辑训练计划' : '新建训练计划'}</h2><div className="mt-5"><Field label="计划名称" value={draft.name} onChange={(name) => setDraft({ ...draft, name })} /></div><div className="mt-7 border-t border-[#edf2ed] pt-6"><h3 className="font-bold">计划安排</h3><div className="mt-4"><Select label="训练日" value={draft.weekday} onChange={(weekday) => setDraft({ ...draft, weekday })} options={weekdayOptions} /></div></div><div className="mt-7 border-t border-[#edf2ed] pt-6"><div className="flex items-center justify-between gap-3"><h3 className="font-bold">训练内容</h3><span className="text-sm font-semibold text-[#438263]">{hasActions ? '记录动作' : '按时长'}</span></div><div className="mt-4 grid grid-cols-2 gap-2 rounded-xl bg-[#edf2ed] p-1"><button className={`rounded-lg px-3 py-2.5 text-sm font-semibold ${!hasActions ? 'bg-white text-[#256a49] shadow-sm' : 'text-[#617065]'}`} onClick={() => setDraft({ ...draft, items: [] })} type="button">只记录时长</button><button className={`rounded-lg px-3 py-2.5 text-sm font-semibold ${hasActions ? 'bg-white text-[#256a49] shadow-sm' : 'text-[#617065]'}`} onClick={() => { if (!hasActions) addAction() }} type="button">记录动作</button></div><div className="mt-4"><Field label="预计时长（分钟）" value={draft.durationMinutes} onChange={(durationMinutes) => setDraft({ ...draft, durationMinutes })} /></div>{hasActions ? <div className="mt-5"><div className="flex items-center justify-between gap-3"><h4 className="font-semibold">训练动作</h4><button className="text-sm font-semibold text-[#256a49]" onClick={addAction} type="button">+ 添加动作</button></div><div className="mt-3 space-y-3">{draft.items.map((item, index) => <div className="rounded-2xl bg-[#f4f7f4] p-4" key={index}><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">动作 {index + 1}</p><button className="text-sm font-semibold text-[#a13d2e]" onClick={() => setDraft({ ...draft, items: draft.items.filter((_, itemIndex) => itemIndex !== index) })} type="button">删除</button></div><label className="mt-3 block"><span className="text-sm font-semibold">动作名称</span><input className="mt-2 w-full rounded-xl border border-[#d6e2d8] bg-white px-3 py-2.5" onChange={(event) => updateItem(index, 'exerciseName', event.target.value)} placeholder="例如：深蹲" value={item.exerciseName} /></label><div className="mt-3 grid gap-3 sm:grid-cols-3"><label><span className="text-sm font-semibold">组数</span><input className="mt-2 w-full rounded-xl border border-[#d6e2d8] bg-white px-3 py-2.5" inputMode="numeric" onChange={(event) => updateItem(index, 'sets', event.target.value)} value={item.sets} /></label><label><span className="text-sm font-semibold">最少次数</span><input className="mt-2 w-full rounded-xl border border-[#d6e2d8] bg-white px-3 py-2.5" inputMode="numeric" onChange={(event) => updateItem(index, 'repsMin', event.target.value)} value={item.repsMin} /></label><label><span className="text-sm font-semibold">最多次数</span><input className="mt-2 w-full rounded-xl border border-[#d6e2d8] bg-white px-3 py-2.5" inputMode="numeric" onChange={(event) => updateItem(index, 'repsMax', event.target.value)} value={item.repsMax} /></label></div></div>)}</div></div> : <p className="mt-4 rounded-xl bg-[#e7f2e9] px-4 py-3 text-sm text-[#346748]">适用于慢走、跑步、骑行等训练；按计划时长完成即可。</p>}<label className="mt-5 flex items-center gap-2 text-sm font-semibold"><input checked={draft.isActive} onChange={(event) => setDraft({ ...draft, isActive: event.target.checked })} type="checkbox" />启用这个计划</label></div><div className="mt-6 flex gap-3"><button className="rounded-xl bg-[#256a49] px-5 py-3 text-sm font-bold text-white disabled:opacity-50" disabled={!userId || saving} type="submit">{saving ? '保存中…' : editingId ? '保存修改' : '创建计划'}</button>{editingId && <button className="rounded-xl bg-[#edf2ed] px-5 py-3 text-sm font-bold text-[#526055]" onClick={() => { setEditingId(null); setDraft(emptyWorkoutPlan()); setMessage('') }} type="button">取消</button>}</div></form></section>
}
...
function Nutrition({ userId, stat, profile }: { userId?: string; stat: { weight: string; waist: string }; profile: ProfileDraft }) {
  ...
  useEffect(() => {
    if (!userId) return
    let active = true
    void Promise.all([loadMeals(userId, ledgerDate), loadNutritionTarget(userId, ledgerDate)]).then(([nextMeals, nextTarget]) => {
      if (!active) return
      setMeals(nextMeals)
      setTarget(nextTarget)
      setPlan(nextTarget)
      setTrainingDay(nextTarget?.trainingDay || false)
    }).catch((error) => { if (active) setMessage(error instanceof Error ? error.message : '读取饮食账本失败') })
    return () => { active = false }
  }, [userId, ledgerDate])
  const totals = mealTotals(meals)
  ...
  const recommend = async () => {
    ...
    try { setPlan(await askNutrition({ action: 'recommend', weightKg: Number(stat.weight), waistCm: Number(stat.waist), trainingDay, profile: { sex: profile.sex, age, heightCm: Number(profile.heightCm), targetWeightKg: Number(profile.targetWeightKg), dietPreferences: textList(profile.dietPreferences), allergens: textList(profile.allergens), foodBudget: profile.foodBudget } }) as DietPlan) }
  ...
  const saveTarget = async () => {
    ...
    setLoading(true); setMessage('')
    try {
      const nextTarget = { ...plan, trainingDay }
      await saveNutritionTarget(userId, ledgerDate, nextTarget)
      setTarget(nextTarget)
      setMessage('每日营养目标已保存。')
    } catch (error) { setMessage(error instanceof Error ? error.message : '保存目标失败') }
    finally { setLoading(false) }
  }
